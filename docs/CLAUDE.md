...
**`canonical` restricts what may be steered, by zoom.** Its azimuth is a detented compass dial — free whilst dragging, then on release it advances a point per `canonicalSnapArm` turned, or springs back inside the arm — and a turn-back within the same drag is a cancel only whilst the drag stayed within a quarter turn. The dial is a zoomed-out thing: close in (`t <= canonicalDialFrom`, `freeCanonicalAzimuth`) the azimuth is free and `canonicalTheta` merely tracks the nearest compass point. Nothing is remembered across the zoom — going out turns you onto the point, and coming back in simply KEEPS that angle. The polar is never clamped by the zoom: TWO tilts are tracked, `polarIn` and `polarOut`, and `shapeCanonicalPolar` blends them by the zoom (smoothstep from `canonicalFlattenFrom`), so the camera travels continuously between them. A drag owns the polar at every zoom; the tilt it leaves is fed back into BOTH ends by how much each weighs there, so the blend goes on passing through it. Entering the mode and a reset set both to the camera's own tilt. Zoom past the outer stop there is none — the radius is only ever a zoom stop.

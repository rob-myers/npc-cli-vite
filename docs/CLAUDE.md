...
**`canonical` restricts what may be steered, by zoom.** Its azimuth is a detented compass dial — free whilst dragging, then on release it advances a point per `canonicalSnapArm` turned, or springs back inside the arm — and a turn-back within the same drag is a cancel only whilst the drag stayed within a quarter turn — and its polar close in is simply the user's own: whatever tilt they leave it at is remembered as `canonicalPolar`. Zoomed out (`t > canonicalFlattenFrom`) the polar is a function of the zoom, eased to birdseye and PINNED by `min/maxPolarAngle`. It is PRESERVED there rather than driven anywhere: it eases out FROM `canonicalPolar` and back TO it — the ease is symmetric, and the aimed zoom-in's tilt (`zoomPan.toPolar`) travels to it too — and is waiting when the zoom comes back in. The azimuth's dial is a zoomed-out thing: close in (`t <= canonicalFlattenFrom`, `freeCanonicalAzimuth`) the azimuth is free, and `canonicalTheta` tracks the nearest compass point. Zooming out remembers the free angle (`freeAzimuth`) and turns to the point AS the zoom goes out; zooming back in turns back to that angle as the zoom comes in, unless the dial was turned to another point meanwhile (`getCanonicalAzimuth`). Zoomed out a shift-drag up or down is a PEEK: whilst the controls report `isRotating` the clamps open to `[shaped, canonicalPolar]`, and on release `shapeCanonicalPolar` seeds a `phi` detent (`seedDetent`, the same lever as the azimuth's) that springs it back to the shaped polar before pinning again (`canonicalTilting`). Zoom past the outer stop there is none — the radius is only ever a zoom stop.

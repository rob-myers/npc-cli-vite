...
**`canonical` restricts what may be steered, by zoom.** Its azimuth is a detented compass dial — free whilst dragging, then on release it advances a point per `canonicalSnapArm` turned, or springs back inside the arm — and its polar close in is simply the user's own: whatever tilt they leave it at is remembered as `canonicalPolar`. Zoomed out (`t > canonicalFlattenFrom`) the polar is a function of the zoom, eased to birdseye and PINNED by `min/maxPolarAngle`, and `controls.enableRotate` goes off so a drag can only pan. Both are PRESERVED there rather than driven anywhere: the polar eases out FROM `canonicalPolar` and back TO it — the ease is symmetric, and the aimed zoom-in's tilt (`zoomPan.toPolar`) travels to it too — whilst the azimuth's detent simply holds the compass point you were on. Both are waiting when the zoom comes back in. `enableRotate` is the right lever because every rotate path honours it, mouse and touch alike — and `setCameraMode` must give it back on the way out, as it does the polar clamps. Zoomed out there is nothing to steer at all: no rotation, and no zoom past the outer stop — the radius is only ever a zoom stop, so a drag can only pan.

**Turns can aim at a point too**, when `rotateToCursor` is on — it is, for `canonical`. `setRotateAbout(clientX, clientY)` meets the cursor's ray with the ground plane through `target` on mouse-down (clamped to `maxDistance`, since a shallow ray lands hundreds of metres out), and `update` then slides the whole rig by `slideAboutPivot`. Turning about the pivot and turning about `target` leave `position - target` identical, so they differ by a pure translation — which is all that helper applies. Mouse only: `handleTouchStartRotate` clears the aim, as touch has no cursor.

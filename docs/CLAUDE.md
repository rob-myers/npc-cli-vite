...
**`canonical` restricts what may be steered, by zoom.** Its azimuth is a detented compass dial — free whilst dragging, then on release it advances a point per `canonicalSnapArm` turned, or springs back inside the arm — and its polar is detented on the single point `canonicalZoomInPolar`. Zoomed out (`t > canonicalFlattenFrom`) the polar is a function of the zoom, eased to birdseye and PINNED by `min/maxPolarAngle`, and `controls.enableRotate` goes off so a drag can only pan. The azimuth is PRESERVED there rather than driven anywhere: the detent simply holds the compass point you were on, and it is waiting when the zoom comes back in. `enableRotate` is the right lever because every rotate path honours it, mouse and touch alike — and `setCameraMode` must give it back on the way out, as it does the polar clamps. Zoomed out there is nothing to steer at all: no rotation, and no zoom past the outer stop — the radius is only ever a zoom stop, so a drag can only pan.

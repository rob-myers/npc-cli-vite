...
export class LinkProvider implements ILinkProvider {
  ...
  constructor(
    _terminal: Terminal,
    _regex: RegExp,
    // _handler: ILink['activate'],
    _handler: (event: MouseEvent, text: string, extraCtxt: ExtraHandlerContext) => void,
    _options: ILinkProviderOptions = {},
  ) {
...
export const computeLink = (y: number, regex: RegExp, terminal: Terminal, matchIndex = 1) => {
  ...
  let match: undefined | null | RegExpExecArray;
  let stringIndex = -1;

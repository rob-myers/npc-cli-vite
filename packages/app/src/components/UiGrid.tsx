...
function UiInstanceMenu({ id, state }: { id: string; state: State }) {
  return (
    ...
    >
      ...
      <Popover.Root>
        <Popover.Trigger data-item-id={id} className="cursor-pointer p-1">
          <XIcon data-icon-type="remove" weight="duotone" className="grayscale" />
        </Popover.Trigger>
        <Popover.Portal>
          <Popover.Positioner side="right" sideOffset={8}>
            <Popover.Popup className="border border-white/30">
              <PopoverArrow iconClassName={cn("fill-background stroke-white/20")} />
              <Popover.Description className="px-2 py-1 bg-background text-on-background">
                <button
                  type="button"
                  className="text-sm cursor-pointer text-red-300/80"
...
function PopoverArrow({
  className,
  iconClassName = "fill-on-background stroke-on-background",
}: {
  className?: string;
  ...
  return (
    ...
    >
      <ArrowSvg className={iconClassName} />
    </Popover.Arrow>
  );
}
